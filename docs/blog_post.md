...
- **Self-improvement is not magic.** DSPy and GEPA rewrote the prompts and found no lift; the real win was architectural, forcing the agent to verify evidence before citing it. And for a regulated agent the proof of that work, every trace and every score, has to live under the same governance as the data it grades, which is why the whole loop runs end to end on Databricks: Unity Catalog, Mosaic AI Vector Search, Foundation Model APIs, and managed MLflow as one governed system.
...
- **Trajectory checks.** Did the agent call the verifier that confirms each cited ID before it wrote the citation? Answered from the recorded tool-call trajectory: every cited control has to carry a verification call that returned true, and the propose/verify/finalize structure is what guarantees that call ran before the answer, because the final step writes only from the verified list. This is the layer that separates an agent that *knew* its citation was good from one that *guessed* and got lucky.
...
We ran all of this on Databricks, and for this kind of agent that is an architecture decision, not a hosting one. The corpus and golden sets live as Unity Catalog Delta tables. Retrieval is Mosaic AI Vector Search, a Delta Sync index built straight off those tables with managed `databricks-gte-large-en` embeddings, so the index is governed by the same Unity Catalog that governs the source table and stays derived from it, not a separately-secured copy living in an outside vector store. The agent calls Foundation Model API endpoints (`databricks-gemini-2-5-flash` for drafting, a second for the judge), so the control text never leaves the platform for an external API key. That gateway is also where the model choice stays open: one endpoint interface fronts frontier models from Anthropic, OpenAI, and Google alongside open-weight models like Qwen and Llama, so a team picks the best model per task, or swaps one out later, without rewriting the agent or shipping its corpus to a new vendor. The portability sweep further down is exactly that swap, run as an experiment instead of asserted. Every trace and score lands on managed MLflow, and because each run records the model endpoint, the dataset, and the variant next to its CLEAR-S scores while the corpus stays in Unity Catalog Delta, a score carries the context that produced it instead of floating free in an external dashboard, and the whole notebook runs as a Job.

The reason that matters is the audit. For a compliance agent the eval evidence is regulated data in its own right, and on Databricks the corpus, the retrieval index, the model endpoint, the trace, and the score live under one governance model, in one workspace, behind one set of permissions. You can argue the parts run anywhere, and the individual parts do. But assemble them from a warehouse plus a bolt-on vector database plus an external trace-and-eval service plus a separate model gateway, and you have rebuilt the stitch-job this harness exists to expose, except now every seam between two products is also a governance boundary your proof-of-correctness has to cross. The trace that proves the agent verified a control would sit in a different system from the control it verified, governed by different rules, exportable by different people. That is the gap that fails an audit, and closing it is what the platform buys you that a pile of individually-compatible components does not.

<!-- FIGURE 3 -->
![Figure 3. The solution as one governed system on Databricks. NIST SP 800-53 lands in Unity Catalog Delta tables; Mosaic AI Vector Search indexes the corpus; the LangGraph agent drafts through Foundation Model API endpoints that front both frontier and open-weight models; managed MLflow captures every trace and CLEAR-S score; DSPy and GEPA optimize on a Job. The dashed enclosure is the point. Unity Catalog governs all of it under one permission model, so no component, and no piece of eval evidence, sits outside the boundary.](./figures/03-databricks-architecture.png)

> **Image-gen prompt** (replace the placeholder above): "Publication-quality flat vector solution-architecture diagram, clean white background, thin 1px charcoal (#1f2933) rules and connectors, single muted teal (#10b981) accent reserved for the governance enclosure and the feedback arrow, neutral gray (#9aa5b1) for secondary labels, no gradients or 3D or drop shadows. All prose set in 'Charter' (the Medium article body typeface); service names and identifiers in a monospace face. Place a recognizable Databricks product icon beside each node, drawn in one consistent flat treatment (the Databricks lava-red glyphs on white): Unity Catalog, Delta Lake, Mosaic AI Vector Search, Foundation Model APIs / Model Serving, MLflow, and Workflows (Jobs). Left-to-right data flow in five stages: (1) a document labeled monospace 'NIST SP 800-53 Rev5' feeding into (2) two Unity Catalog Delta tables labeled 'corpus' and 'golden'; (3) a Mosaic AI Vector Search box labeled 'Delta Sync index · gte-large-en' reading from the corpus table; (4) a LangGraph agent box showing a small six-node chain 'parse to classify to retrieve to propose to verify to finalize', with a side call-out to a Foundation Model APIs box labeled 'Anthropic / OpenAI / Gemini / Qwen behind one endpoint'; (5) a managed MLflow box capturing 'traces + CLEAR-S scores', feeding a 'DSPy + GEPA optimizer (Job)' box and a 'Model Serving + inference tables' box. A single dashed teal rounded rectangle labeled 'Unity Catalog · governance + lineage' encloses ALL of these nodes, with a small monospace note on its edge 'one permission model, one governed boundary'. A teal feedback arrow runs from 'inference tables' back to the 'golden' table, labeled 'production failures become regression cases'. Caption styled like a journal figure. 16:9."
...
Now the trajectory scorer can prove, from the recorded tool-call trajectory, that every citation traces to a verification that returned true before the answer was written. Here are the measured numbers on the NIST run, 20 questions, scored 0 to 1:
...
<!-- FIGURE 5 -->
![Figure 5. The loop as one governed surface on Databricks. Each stage maps to a managed service, and the arrow back from monitoring is the point: a production failure re-enters as the next regression case instead of disappearing into a log.](./figures/05-databricks-loop.png)
...
> **Note on figures.** Figures 1 and 2 are already rendered into `./figures/` from the real run numbers by `docs/figures/make_figures.py`; re-run that script to regenerate them. Figures 3, 4, and 5 are placeholders: each carries an image-gen prompt in the blockquote beneath it, so generate the figure, drop it into `./figures/` under the referenced filename, and delete the prompt blockquote. All five share one house style (white background, thin charcoal rules, single muted-teal accent reserved for the signal that matters, flat vector, and all text set in Charter, the Medium article body typeface) so the set reads as one figure family. Figure 3 additionally carries Databricks product icons for each managed service.
